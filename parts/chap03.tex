...
设备各工况轴向载荷及倾覆力矩汇总列于表 \ref{tab:skirt-loads}。

\begin{table}[!htb]
  \centering
  \caption{裙座底部载荷汇总}
  \label{tab:skirt-loads}
  \small\renewcommand{\arraystretch}{1.15}
  \setlength{\tabcolsep}{8pt}
  \begin{tabularx}{\textwidth}{@{}l c c c c@{}}
    \toprule
    \textbf{载荷项} & \textbf{符号} & \textbf{操作工况} & \textbf{空重工况} & \textbf{水压试验} \\
    \midrule
    设备总质量 / kg          & $m$      & 7245  & 4245  & 9745  \\
    轴向载荷 / kN            & $N$      & 71.1  & 41.6  & 95.6  \\
    风弯矩 / kN$\cdot$m       & $M_w$    & 6.3   & 6.3   & ——    \\
    地震弯矩 / kN$\cdot$m     & $M_e$    & 13.3  & 13.3  & ——    \\
    组合弯矩 $M_{\max}$ / kN$\cdot$m & —— & 13.3  & 13.3  & ——    \\
    \bottomrule
  \end{tabularx}
\end{table}
...
焊接节点详图编号体系见表 \ref{tab:weld-joint-id}，与 CAD 零部件图中焊接节点标识一一对应。

\begin{table}[!htb]
  \centering
  \caption{焊接节点详图编号}
  \label{tab:weld-joint-id}
  \small\renewcommand{\arraystretch}{1.2}
  \setlength{\tabcolsep}{8pt}
  \begin{tabularx}{\textwidth}{@{}c >{\centering\arraybackslash}X c@{}}
    \toprule
    \textbf{节点编号} & \textbf{接头名称} & \textbf{焊接方法} \\
    \midrule
    WJ-01 & 筒体纵缝                      & SAW \\
    WJ-02 & 筒体与上封头环缝               & SAW \\
    WJ-03 & 筒体与下封头环缝               & SAW \\
...
壳体计算厚度 $\delta = 4.57$ mm（见式 \eqref{eq:cyl-delta}）。接管计算厚度按内压圆筒公式（以外径为基准）：
\[
\delta_t = \frac{P_c D_o}{2[\sigma]^t_n + P_c} = \frac{0.80 \times 610}{2 \times 140 + 0.80} = 1.74\ \text{mm}
\]

接管有效厚度 $\delta_{et} = \delta_{nt} - C_2 = 12 - 3.0 = 9.0$ mm；强度削弱系数 $f_r = [\sigma]^t_n / [\sigma]^t = 140/186 = 0.753$。
...
各主要开孔的补强计算结果汇总于表 \ref{tab:reinforcement}。DN600 人孔其接管厚壁自补强已满足等面积补强准则；DN100 开孔区域同理满足；DN50 及以下小口径仪表接管因开孔直径小，壳体有效厚度提供的多余金属已覆盖削弱面积，不需另行补强。
